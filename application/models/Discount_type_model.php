...
class Discount_type_model extends CI_Model {
	public function __construct(){
       ...
       // $ci &=get_instance();
        $this->load->database();
        $this->tbl='sr_discount_type';
        $this->prefix='disc_type_';
    }
	...
	function getOne_byid($aId)//---------------OK
	{
		...
		if (is_array($aId))
			...
			$this->db->where($this->tbl.'.'.$this->prefix.'id', $aId);
		// $this->db->join('sr_cate_service', 'sr_cate_service.cate_id = sr_service.serv_cate_id');
	...
	function Delete($id,$path='')//---------------OK
	{
		...
		$listPhoto=json_decode($get['disc_type_photo']);
		if(!empty($listPhoto)){
...
/* End of file Discount_type_model.php */
/* Location: ./application/models/Discount_type_model.php */