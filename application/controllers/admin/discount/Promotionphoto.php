...
class Promotionphoto extends CI_Controller {
	...
	{
		// echo "string";
		$list=$this->prophoto->getList();
		// $dataout=(!empty($list))?$this->myhtml->DeQuyCate(0,$list,'','dropdown'):'';
		// $this->myclass->indulieu($dataout);die();
		$dataout=$list;
		$data=array(
			'info'=>array(
	...
	public function add()
	{
		...
		// $dataSelect=(!empty($datacate))?$this->myhtml->dropdown('Root',$this->myhtml->DeQuyCate(0,$datacate,'','dropdown')):NULL;
					...
					if(!empty($data['btn']['_Submit']))//Save click
					{
						...
						if (!empty($_FILES['photo']['name'][0]))
						{
								...
								for ($i=0; $i < $TotalFile ; $i++) { 
										$_FILES['photo']['name'] = random_string('alnum',6)."-".$files['photo']['name'];
										...
										if(!($this->upload->do_upload('photo')) || $files['photo']['error'] !=0)
				            {
				      ...
				      //       } //End IF Upload file
								...
								$data['filename']=$filenamesave[0];
								
						}
						$this->prophoto->Add($data);
   ...
   public function uploadImage() { 
      ...
      $config['upload_path']   = './upload/promotion'; 
