...
{block name='Page_content'}
		...
		<div class="box">
		  ...
		  <div class="box-body">
		  	<p><a href="http://nails.tk/{$pathControl}add"><button type="button" class="btn btn-primary"><i class="fas fa-plus"></i> Add {$page_name} </button></a></p>
            ...
            <tr>
              <td>{$item.a_id}</td>
              <td>{$item.a_title}</td>
              <td><img src="http://nails.tk/upload/{$item.a_photo}" class="img-responsive" alt="Image" style="height: 50px;"></td>
              {* <td>{if $item.a_active eq 1}Active{else}Disable{/if}</td> *}
              {* <td>{$item.a_timecreate}</td> *}
              <td>
              	{* <a href="http://nails.tk/{$pathControl}edit/{$item.a_id}"><span class="fa fa-pencil-square-o btn btn-primary"></span></a> *}
              	<a href="http://nails.tk/{$pathControl}delete/{$item.a_id}"><span class="fa fa-trash btn btn-danger"></span>	</a>
            	</td>
            </tr>
            {/foreach}
	        </tbody>
...
/* Location: ./application/views/templates/backend/default/article/job/list.tpl */ *}